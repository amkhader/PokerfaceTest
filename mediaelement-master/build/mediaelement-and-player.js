		...
		buildoverlays: function(player, controls, layers, media) {
			...
			redConfused =
				...
				document.getElementById("redCon").onclick = function(){
					...
					var JargonFun = '<script>function jargonFunction() {var word = JargonWords[currentSubNum]; window.csvResult.push("Jargon "+track.entries.times[currentSubNum][\'identifier\']);makeLink(window.csvResult,"input"); document.getElementById("container").innerHTML = word + \': \\n <div class= "definition"></div>  \\n <h3>Did this help? Why/Why not?</h3> <textarea id = \"textArea\"></textarea><div onclick="savetextareaFeedback()" id="submit" type="button" class="popup_close">Submit</div>\'; $.getJSON("http://ar.wikipedia.org/w/api.php?format=json&action=query&prop=extracts&exintro=&explaintext=&titles=" + "أيون" + "&callback=?" , function(data){console.log(data); var response2 = ""; var response = $.parseJSON(((data.query).pages));for (var id in data.query.pages) { response2 = data.query.pages.id.extract;} console.log(response2); $(\'.definition\').html("<div>" + response2 + "</div>");});} </script>';
