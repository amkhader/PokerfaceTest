		...
		buildoverlays: function(player, controls, layers, media) {
			...
			redConfused =
				...
				document.getElementById("redCon").onclick = function(){
					...
					var JargonWinFun = '<script>function jargonpopup(){ var jargon = " "; console.log(currentSubNum); var j = $.inArray(currentSubNum, JargonSubNum); if (j > -1){ jargon = JargonWords[j]; htmlQ2 = "هل تشعر الخلط من ".concat(JargonWords[j]).concat("?"); $("h1").html(htmlQ2);$("h2").html("Are you confused by".concat(JargonWords[j]).concat("?"));$("#container").html("<button onclick = \'jargonFunction(jargon)\' id= \'button1\'>نعم</button> <button onclick = \'MTEpopup()\' id = \'button2\'>لا</button>");}else{MTEpopup();}}</script>';
					var JargonFun = '<script>function jargonFunction(word) {window.csvResult.push("Jargon "+track.entries.times[currentSubNum][\'identifier\']);makeLink(window.csvResult,"input"); document.getElementById("container").innerHTML = word + \': \\n <div class= "definition"></div>  \\n <h3>Did this help? Why/Why not?</h3> <textarea id = \"textArea\"></textarea><div onclick="savetextareaFeedback()" id="submit" type="button" class="popup_close">Submit</div>\'; $(\'.definition\').load(\' www.almaany.com/ar/dict/ar-ar/\' + word + \'.result_meaning\');} </script>';
