		...
		buildoverlays: function(player, controls, layers, media) {
			...
			redConfused =
				...
				document.getElementById("redCon").onclick = function(){
					...
					var htmlEN = '<h2 id = "htmlEN">(Are you confused by something in this frame?)</h2>';
					...
					var htmlButtonsRewind = '<div id="container"><button onclick="jargonpopup()" id="button1">نعم (Yes) </button> <button onclick="rewindFunction()" id = "button2">لا، إطار سابق (No, a previous frame)</button></div>';
					...
					var JargonWinFun = '<script>function jargonpopup(){ window.csvRewinds.push(currentSubNum + "," + numRewinds);numRewinds=0;console.log(currentSubNum); var j = $.inArray(currentSubNum, JargonSubNum); if (j > -1){ jargon = JargonWords[j]; htmlQ2 = ": هل تشعر الحيرة من جانب".concat(jargon).concat("?"); $("h2").html("Are you confused by".concat(JargonWords[j]).concat("?"));$("#container").html("<button onclick = \'jargonFunction()\' id= \'button1\'>نعم  (Yes)</button> <button onclick = \'MTEpopup()\' id = \'button2\'>لا (No) </button>");}else{MTEpopup();}}</script>';
					...
					var MTEpopup = '<script>function MTEpopup() {$("h1").html("هل تشعر بالحيرة من جانب الترجمة؟") ;  $("h2").html(\'Are you confused by the translation?\');$("#container").html("<button onclick = \'MTEFunction()\' id= \'button1\'>نعم</button> <button onclick = \'Textpopup()\' id = \'button2\'>لا</button>");} </script>';
					...
					popup.open(htmlQ1.concat(htmlEN).concat(htmlButtons).concat(htmlButtonsRewind).concat(ResultsFile).concat(ResultsLink).concat(RewindFun).concat(JargonWinFun).concat(JargonFun).concat(MTEpopup).concat(MTEFunction).concat(Textpopup).concat(SaveInput).concat(SaveFeedback).concat(FeedbackButton), 'html', $('a.button'));
					//concat(htmlEN).
		...
		displayCaptions: function() {
						...
						if ((track.entries.text[i]).search("sub") < 0){
							...
							//console.log("wcounter: " + wcounter);
										 for (index = 0; index < theOldText.length; index++) {
												  ...
												  //console.log(thePosIndexArray[p]);
												
												  //console.log(('#sub'+ i + 'in' + index))
												  p++;
										  }
									track.entries.text[i] = theNewText.join(" ");	
									//console.log(track.entries.text[i]);
			...
			if (counterf == wcounter && doneFor[counterf].done != 1) {
				 //console.log("times1 "+doneFor[counter].done);
				  ...
				  csvFeedbackContent += window.csvFeedback.join("\n");
