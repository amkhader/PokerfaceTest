		...
		buildoverlays: function(player, controls, layers, media) {
			...
			redConfused =
				...
				document.getElementById("redCon").onclick = function(){
					...
					var JargonWinFun = '<script>function jargonpopup(){ var word = " "; console.log(currentSubNum); var j = $.inArray(currentSubNum, JargonSubNum); if (j > -1){ word = JargonWords[j]; htmlQ2 = "هل تشعر الخلط من ".concat(JargonWords[j]).concat("?"); $("h1").html(htmlQ2);$("h2").html("Are you confused by".concat(JargonWords[j]).concat("?"));$("#container").html("<button onclick = \'jargonFunction(word)\' id= \'button1\'>نعم</button> <button onclick = \'MTEpopup()\' id = \'button2\'>لا</button>");}else{MTEpopup();}}</script>';
