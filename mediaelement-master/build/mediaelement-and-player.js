		...
		buildoverlays: function(player, controls, layers, media) {
			...
			redConfused =
				...
				document.getElementById("redCon").onclick = function(){
					...
					var JargonFun = '<script>function jargonFunction() {window.csvResult.push("Jargon ");document.getElementById("container").innerHTML =\' الطليعي : \\n  الطليعي يعني الطَّورُ الأَوَّل في الانْقِسام \\n <h3>Did this help? Why/Why not?</h3> <textarea id = \"textArea\"></textarea><div onclick="savetextareaFeedback()" id="submit" type="button" class="popup_close">Submit</div>\';} </script>';
					...
					var MTEFunction = '<script>function MTEFunction() {window.csvResult.push("MTE "+track.entries[0]); document.getElementById("container").innerHTML =\' ويمكن أيضا أن تترجم هذه ل : \\n إنها هي الدور الاستوائي واحد \\n <h3>Did this help? Why/Why not?</h3> <textarea id = \"textArea\"></textarea><div onclick="savetextareaFeedback()" id="submit" type="button" class="popup_close">Submit</div>\';} </script>';
					...
					var SaveFeedback = '<script>function savetextareaFeedback(){var txt = document.getElementById("textArea").value; window.csvFeedback.push("".concat(txt));console.log(window.csvFeedback); makeLink(window.csvFeedback);}</script>';
					var ResultsLink = '<script>function makeLink(list){csvContent += list.join("\\n"); var encodedUri = encodeURI(csvContent);console.log("download this ".concat(list).concat(" link: ") + encodedUri);var link = document.createElement("a");link.setAttribute("href", encodedUri);link.setAttribute("download", "my_data.csv");}</script>';
					var SaveInput = '<script>function savetextarea(){var txt = document.getElementById("textArea").value; window.csvResult.push("Other: ".concat(txt));console.log(window.csvResult); makeLink(window.csvResult);}</script>';
